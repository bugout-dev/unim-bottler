...
class BottlerFacet:
    ...
    def set_terminus_pool_uri(self, pool_id: int, uri: str, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.setTerminusPoolURI(pool_id, uri, transaction_config)
...
def handle_set_terminus_pool_uri(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = BottlerFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.set_terminus_pool_uri(
        pool_id=args.pool_id, uri=args.uri, transaction_config=transaction_config
    )
    print(result)
...
def generate_cli() -> argparse.ArgumentParser:
    ...
    set_terminus_pool_uri_parser = subcommands.add_parser("set-terminus-pool-uri")
    add_default_arguments(set_terminus_pool_uri_parser, True)
    set_terminus_pool_uri_parser.add_argument(
        "--pool-id", required=True, help="Type: uint256", type=int
    )
    set_terminus_pool_uri_parser.add_argument(
        "--uri", required=True, help="Type: string", type=str
    )
    set_terminus_pool_uri_parser.set_defaults(func=handle_set_terminus_pool_uri)
